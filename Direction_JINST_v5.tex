...
providing energy resolutions of $\sim$5\%$/\sqrt{E(MeV)}$\cite{kam2013,borexino}. This is roughly a factor of
two better than water Cherenkov detectors, the other developed
technology that can be economically scaled to these large
masses. However, this energy resolution is much poorer than other technologies being used for $0\nu\beta\beta$ searches: Ge detectors\cite{gerda2013}, Te bolometers, liquid Xe time projection chambers (TPCs)\cite{EXO2012} and high pressure gaseous Xe TPCs\cite{NEXTsipm}.  

Scintillation light is isotropic and any information on the primary particle's direction is lost.  Cherenkov light is also
produced. Most is absorbed and re-emitted as part of the
...
scintillating neutrino detectors. This is a different technique and application than the direction reconstruction described for high-energy neutrino interactions\cite{john} or that for neutrons from inverse beta decay\cite{chooz,dcDirection}.  We then use these results as input into a traditional direction reconstruction developed for water Cherenkov detectors. Since the reconstruction of the direction of $\sim$1~MeV particles has not been achieved before, we will start these studies with the most simple case of a single particle at the center of the detector. We will also start with an easier test case of a 5~MeV electron like that from a $^{8}$B solar neutrino interaction. With the higher photon statistics at this energy, we will verify the technique and the different detector parameters that effect it. We will then lower the energy to that relevant to a single electron in a $0\nu\beta\beta$.  Since these simpler scenarios are successful, this justifies future work, specifically on the reconstruction of the two simultaneous electrons throughout the detector volume and the study of the sensitivity of a detector instrumented to take advantage a directional signal to both $0\nu\beta\beta$ and to new physics.
...
\end{equation}
where x is the distance travelled by the particle, $n(\lambda)$ is the wavelength-dependent index of refraction and
$\beta$ is the velocity of the incoming particle. 
...
information. For the example scintillator above, n(400nm) is 1.459\cite{OlegThesis}. The absorption cutoff is between 360~nm and 370~nm where the attenuation length rapidly drops from 6.5~m to 0.65~m. For a 5~MeV electron, this yields 744 Cherenkov photons per event, and dropping down to a 1~MeV electron yields 89.3 Cherenkov photons per event when integrating from the cutoff wavelength at 360~nm to 550~nm, the point above which a negligible number of photons is produced.
...
\end{equation}
This means photons right above the scintillation cutoff at 370~nm will travel at 0.1914~m/ns while those at longer wavelengths for example 600~nm travel at 0.2031 m/ns. Since on average the undisturbed Cherenkov photons have longer wavelengths, they will arrive before the
...
electrons follow distributions of energies and angular correlations which depend on the underlying decay $0\nu\beta\beta$ mechanism\cite{newphysics0nuBB, SuperNEMO}.
For the basic mechanism of light Majorana neutrino exchange, a probable case is the equal division of energy between back-to-back
electrons\cite{phasespace, SuperNEMO}. This case is shown in figure \ref{detector_view}
...
of the $^{238}$U and $^{232}$Th decay chains, isotopes with Q-values above the $^{208}$Tl  2.6~MeV gamma are preferred, however slightly lower Q-values can be tolerated if there is sufficient background rejection. There are hundreds of candidate $0\nu\beta\beta$ isotopes\cite{tabledbb},
...
bulk volume and optical scattering, specifically Rayleigh scattering, have not yet been included by default. A test simulation shows that their effects are negligible. The reason is that the cutoff is very steep below 360~nm and almost no photons reach the sphere, so optical scattering makes no difference. Above about 395~nm, the attenuation length is greater than 20~m so both scattering and absorption are not very likely and scattering is negligible. The intermediate region is rather small.  This is the conclusion of a more detailed study found in Ref.~\cite{Wurm:2010ad}. 
...
system: the travel time of the particle, the time constants of the scintillation process, chromatic dispersion and the timing of the photodetector.  First, the simulated travel time of an initial 5~MeV electron is 0.108$\pm$0.015~ns. This corresponds to an average travel distance of 3.1~cm and a distance from the origin of 2.6~cm. We can define an effective Cherenkov production threshold and the time until the electron drops below threshold is 0.106$\pm$0.015~ns. We note that due to scattering the final direction of the electron before it stops does not correspond to the initial direction, however in the steps before the electron drops below Cherenkov threshold the scattering angle is small and therefore the Cherenkov light encodes the direction of the primary electron. The scattering physics is handled by Geant4's ``Multiple Scattering" process which is valid down to 1~keV where atomic shell structure becomes important\cite{geant4scatt}. In the energy range important for $0\nu\beta\beta$, a 1.4~MeV electron travels a total distance of 0.8~cm and has a distance from the origin of 0.6~cm in 0.030$\pm$0.004~ns  and takes 0.028$\pm$0.004~ns to drop below Cherenkov threshold. The scattering follows the same pattern. 
...
transit-time spread. The higher energy provides larger photon statistics with which to evaluate the method and debug the simulation. The true hit time distributions of photoelectrons
...
electrons from this early light. To start, we are neglecting the effects of position dependence. In future work the arrival times can be corrected by the time of flight from the reconstructed vertex and the position and reconstruction fitted simultaneously. The vertex reconstruction uncertainty leads to an additional smearing of the arrival time distribution of 0.15~ns for every 3~cm of position reconstruction uncertainty.
